...
min_avg_lost = int(input())
lost_list = list(map(int, input().split()))
res = []
for i in range(len(lost_list)):
    max_len = 0
    for j in range(i + 1, len(lost_list) + 1):
        tmp_list = lost_list[i:j]
        if len(tmp_list) > 1 and sum(tmp_list) / len(tmp_list) <= min_avg_lost:
            max_len += 1
    if max_len > 0:
        res.append(f'{i}-{i + max_len}')
print(' '.join(res))